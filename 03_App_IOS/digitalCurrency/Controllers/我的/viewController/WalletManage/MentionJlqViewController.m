...
#import "AccountSettingInfoModel.h"  // john

@interface MentionJlqViewController ()<UITextFieldDelegate,AddMentionCoinAddressViewControllerDelegate>
...
@property (weak, nonatomic) IBOutlet UITextField *shopPassword;//商城账号密码  john
@property (weak, nonatomic) IBOutlet UILabel *shopPasswordLabel;
...
@property (weak, nonatomic) IBOutlet UIButton *eye1Button;
- (IBAction)openEyeAction:(id)sender;

- (IBAction)openEye1Action:(id)sender;
...
@property(nonatomic,strong) AccountSettingInfoModel *accountInfo; // john

@end

@implementation MentionJlqViewController
...
- (void)viewDidLoad {
    ...
    self.mentionMoneyAddress.placeholder = [[ChangeLanguage bundle] localizedStringForKey:@"inputShopAccount" value:nil table:@"English"];
    self.numTextField.keyboardType = UIKeyboardTypeDecimalPad;
    self.moneyPassword.secureTextEntry = YES;    [self mentionCoinInfo];
    ...
    self.mentionCoinAdddressLabel.text = [[ChangeLanguage bundle] localizedStringForKey:@"shopAccount" value:nil table:@"English"];
...
//    [self.QrCodeButton setTitle:[[ChangeLanguage bundle] localizedStringForKey:@"addAddress" value:nil table:@"English"] forState:UIControlStateNormal];
    self.QrCodeButton.hidden = YES; // john
    ...
    self.shopPasswordLabel.text = @"商城账号密码";
    ...
    self.mentionMoneyAddress.text = @"";  // john
}
...
-(void)submitInfo{
    ...
    [MineNetManager mentionCoinApplyForUnit:self.model.unit withAddress:self.mentionMoneyAddress.text withAmount:self.numTextField.text withFee:self.poundageNum.text withRemark:remark withJyPassword:self.moneyPassword.text mobilecode:self.mobilecodeTF.text
        withShopPassword:self.shopPassword.text
    ...
    [MineNetManager accountSettingInfoForCompleteHandle:^(id resPonseObj, int code) {
        ...
        if (code) {
            if ([resPonseObj[@"code"] integerValue] == 0) {
                ...
                self.mentionMoneyAddress.text = self.accountInfo.username;
...
- (IBAction)openEye1Action:(id)sender {
    self.eye1Button.selected = !self.eye1Button.selected;
    self.shopPassword.secureTextEntry = !self.eye1Button.selected;
}
@end
